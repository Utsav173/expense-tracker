...
export function calcPercentageChange(array: any[]) {
  // // Calculate percentage change for income values
  // array = array.filter((item) => !isNaN(item));

  // let percentageChanges = [];
  // for (let i = 1; i < array.length; i++) {
  //   let percentageChange = ((array[i] - array[i - 1]) / Math.abs(array[i - 1])) * 100;
  //   percentageChanges.push(percentageChange);
  // }

  // let finalChange =
  //   percentageChanges.reduce((a: number, b: number) => a + b, 0) / percentageChanges.length;
