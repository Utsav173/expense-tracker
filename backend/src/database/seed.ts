  ...
  for (let j = 0; j < account.length; j++) {
    ...
    for (let i = 0; i < number; i++) {
      ...
      const randomDate = new Date(
        Date.now() - Math.floor(Math.random() * 20 * 365 * 24 * 60 * 60 * 1000),
      );
      ...
      if (type === 'income') {
        ...
        updatedAnalyticsData.incomePercentageChange =
          (updatedAnalyticsData.income! / updatedAnalyticsData.balance!) * 100;
      } else {
        updatedAnalyticsData.balance! -= randomAmount;
        ...
        updatedAnalyticsData.expensesPercentageChange =
          (updatedAnalyticsData.expense! / updatedAnalyticsData.balance!) * 100;
      }
    }
    ...
    await db
      .update(Account)
      .set({
        balance: sql`${Account.balance} + ${totalIncome - totalExpenses}`,
      })
      .where(eq(Account.id, account[j].id as any));
  }
  console.log(chalk.ansi256(42).bold(`data generation done ${Date.now() - startTime}ms`));
  console.log('  ');
