...
export function createInvestmentTools(userId: string) {
  return {
    addInvestment: tool({
      ...
      parameters: z.object({
        investmentAccountIdentifier: z
          .string()
          .min(1)
          .describe('Name or ID of the investment account holding this investment. Example: "My Brokerage" or "inv_acc_123"'),
        symbol: z
          .string()
          .min(1)
          .describe(
            "The stock ticker or mutual fund symbol (e.g., 'RELIANCE.NS', 'INFY'). if found symbol is not accruate, you can utilized 'searchStockSymbols' tools to get correct stock symbol. Example: "GOOGL""),
        ...
        purchaseDateDescription: z
          .string()
          .describe("Date of purchase (e.g., 'today', '2024-01-15'). Example: "yesterday" or "2023-11-20""),
      }),
      execute: async ({
    ...
    listInvestments: tool({
      ...
      parameters: z.object({
        investmentAccountIdentifier: z
          .string()
          .min(1)
          .describe('Name or ID of the investment account. Example: "My Brokerage" or "inv_acc_123"'),
        limit: z.number().int().positive().optional().default(20).describe('Max results. Example: 10'),
      }),
    ...
    executeConfirmedUpdateInvestment: tool({
      ...
      parameters: z.object({
        investmentId: z.string().describe('Exact unique ID of the investment. Example: "inv_abc123"'),
        newShares: z.number().positive().optional().describe('New number of shares (optional). Example: 15'),
        newPurchasePrice: z
          .number()
          .nonnegative()
          .optional()
    ...
    executeConfirmedUpdateDividend: tool({
      ...
      parameters: z.object({
        investmentId: z.string().describe('Exact unique ID of the investment. Example: "inv_def456"'),
        newTotalDividend: z.number().nonnegative().describe('New total dividend amount received. Example: 50.00'),
      }),
    ...
    executeConfirmedDeleteInvestment: tool({
      ...
      parameters: z.object({
        investmentId: z.string().describe('Exact unique ID of the investment to delete. Example: "inv_ghi789"'),
      }),
