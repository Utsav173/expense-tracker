...
const getDailyData = async (userId: any) => {
  try {
...
accountRouter.get('/dashboard', authMiddleware, async (c) => {
  try {
    ...
    if (
      ...
      accountsInfo[0].income === 0 &&
      accountsInfo[0].expense === 0
    ) {
      return c.json({
        accountsInfo,
    ...
    const overallData =
      dashboardDataQuery.length > 0 ? dashboardDataQuery[0] : {};
...
accountRouter.get('/searchTerm', authMiddleware, async (c) => {
  ...
  const result = await db
    .execute(sql``.append(selectQuery).append(whereQuery))
    .catch((err) => {
      throw new HTTPException(400, {
        message: err instanceof Error ? err.message : 'Something went wrong',
      });
...
accountRouter.get('/get-shares', authMiddleware, async (c) => {
  ...
  const accountData = await db
    ...
    .leftJoin(Analytics, eq(Analytics.account, Account.id))
    .orderBy(
      sortOrder === 'desc' ? desc(Account[sortBy]) : asc(Account[sortBy])
    )
    .limit(+limit)
    .offset(+limit * (+page - 1))
    .catch((err) => {
...
accountRouter.get('/customAnalytics/:id', authMiddleware, async (c) => {
  ...
  const result: any[] = await db
    .execute(
      sql`
      SELECT
      ct.total_income as "income",
      ct.total_expense as "expense",
      ct.total_balance as "balance",
      100 * ((ct.total_balance - COALESCE(pt.prev_total_balance, 0)) / COALESCE(pt.prev_total_balance, 1)) AS "BalancePercentageChange",
      100 * ((ct.total_income - COALESCE(pt.prev_total_income, 0)) / COALESCE(pt.prev_total_income, 1)) AS "IncomePercentageChange",
      100 * ((ct.total_expense - COALESCE(pt.prev_total_expense, 0)) / COALESCE(pt.prev_total_expense, 1)) AS "ExpensePercentageChange"
    FROM (
      SELECT
...
accountRouter.get('/', authMiddleware, async (c) => {
  ...
  return c.json({
    ...
    limit: +limit,
    page: +page,
    totalPage: Math.ceil(accounts.length / +limit),
  });
});
...
accountRouter.get('/:id/statement', authMiddleware, async (c) => {
  ...
  const { startDate, endDate, numTransactions } = c.req.query();
  ...
  const previousAnalytics = await db.query.Analytics.findFirst({
    where: eq(Analytics.account, id),
  ...
  if (previousAnalytics) {
    const previousIncome: number = previousAnalytics.income ?? 0;
    const previousExpense: number = previousAnalytics.expense ?? 0;
