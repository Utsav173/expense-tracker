...
const AddImportFile = ({ handleSubmitFile, loading }) => {
  ...
  return (
    <Box component='form' noValidate onSubmit={e => handleSubmitFile(e)} sx={{ mt: 3 }}>
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <FormControl fullWidth>
            ...
            <FormHelperText>Select an account to import transactions</FormHelperText>
          </FormControl>
        </Grid>
        <Grid item xs={12}>
          <label htmlFor='importfile'>
            <Box
              sx={{
                ...
                py: 1,
                my: 'auto',
                borderColor: theme => (theme.palette.mode === 'dark' ? '#5f5f5f' : '#7e7e7e')
              }}
            >
              <AttachFileIcon />
              <Typography variant='body1' component='div'>
                {importFile ? `${importFile.name?.slice(0, 20)}...` : 'Select transactions file'}
              </Typography>
            </Box>
            {importFile && (
              <FormHelperText>
      ...
      <Button
        ...
        sx={{
          mt: 3,
          mb: 2,
          backgroundColor: theme => (theme.palette.mode === 'light' ? '#001f37' : '#353f46'),
