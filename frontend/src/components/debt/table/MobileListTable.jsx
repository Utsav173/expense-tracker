...
const MobileListTable = () => {
  const theme = useTheme()
  const dispatch = useDispatch()
  const isXS = useMediaQuery('(min-width:667px)')
  const { debtsList } = useSelector(state => state.debtPage)
  return (
    <Paper
    ...
    >
      {debtsList &&
        debtsList.map((transaction, index) => (
          <Accordion
            key={transaction.id}
            sx={{
              '&.Mui-expanded': {
            ...
            >
              ...
              >
                {isXS ? (
                  ...
                  <Typography variant='body1' noWrap={false}>
                    {transaction.text.toString().length > 20
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={6}>
                  <Stack width={'100%'} direction={'row'}>
                    <EditDebt transaction={transaction} />
                    <IconButton
