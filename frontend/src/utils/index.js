...
export const currencyFormat = (value, notation) => {
  return new Intl.NumberFormat('en-US', {
    ...
    currency: 'INR',
    notation: notation
  }).format(typeof value === 'number' ? value.toFixed(2) : parseFloat(value).toFixed(2))
}
...
export const handleValidation = formData => {
  ...
  if (!passwordRegex.test(password.trim())) {
    toast.error('Password should contain at least one letter, one digit, and one special character')
    return false
  }
...
export const getDurationInterval = {
  thisMonth: 'from last month',
  thisWeek: 'from last week',
  thisYear: 'from last year',
  today: 'from previous day',
  all: 'from last 5 years'
}
