      ...
      .addCase(fetchDashboardData.fulfilled, (state, action) => {
        ...
        if (action.payload.accountsInfo.length > 0) {
          ...
          state.balanceChartData = {
            series: action.payload?.accountsInfo?.map(account => account.balance),
            labels: action.payload.accountsInfo.map(account => account.name)
          }
        }
        if (action.payload.incomeChartData && action.payload.incomeChartData.length > 0) {
          state.iSparkLineData = action.payload.incomeChartData
        }
        if (action.payload.expenseChartData && action.payload.expenseChartData.length > 0) {
          state.eSparkLineData = action.payload.expenseChartData
        }
        if (action.payload.balanceChartData && action.payload.balanceChartData.length > 0) {
...
export const selectByCategoryData = state => state.dashboardPage.byCategoryData
...
export const selectBalanceChartData = state => state.dashboardPage.balanceChartData
